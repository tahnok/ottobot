...
async def setup(bot: MeshBot) -> None:
    assert bot.db_path is not None
    await asyncio.to_thread(_init_db, bot.db_path)
...
async def welcome(ctx: Context) -> str | None:
    ...
    if not name:  # channel message with no recoverable name
        return None
    assert ctx.bot is not None and ctx.bot.db_path is not None
    now = datetime.now(timezone.utc).isoformat()
    is_new = await asyncio.to_thread(_record, ctx.bot.db_path, name, now)
